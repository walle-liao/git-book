...
`git status -s`  # 查看变更文件和变更状态，那些标识为`UU`的文件就是需要解决的冲突文件    
...
`git apply --check test.patch`  # 检查补丁文件
`git apply test.path -3` # 冲突时，强行合并，会产生冲突

`git rebase --onto HEAD~5 HEAD~3 master` # 特殊的方式删除提交点（不常用）    
> E - F - G - H - I - J(HEAD)  master 上面的命令效果就是以 E 为基准点，G 之后的所有提交点（HIJ）重新提交，提交之后的效果：E - H' - I' - J'
